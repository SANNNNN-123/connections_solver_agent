...
@llm_interface_registry.register("gemini")
class LLMGeminiInterface(LLMInterfaceBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.word_analyzer_llm = ChatGoogleGenerativeAI(
            model=self.word_analyzer_llm_name,
            temperature=self.temperature,
            max_output_tokens=self.max_tokens,
        )

        self.image_extraction_llm = ChatGoogleGenerativeAI(
            model=self.image_extraction_llm_name,
            temperature=self.temperature,
            max_output_tokens=self.max_tokens,
        )

        self.workflow_llm = ChatGoogleGenerativeAI(
        ...
        self.embedding_model = GoogleGenerativeAIEmbeddings(
            model=self.embendding_model_name,
        )
    ...
    async def choose_embedvec_item(self, anchor_definition: str, candidate_definitions: list) -> dict:
        ...
        try:
            ...
            if "candidategroup" not in result or "explanation" not in result:
                ...
                # Try to map the keys from the gemini format to the openai format
